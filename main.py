...
def get_simulation_state(
    ...
    print(f"DEBUG: get_simulation_state called with city_name={city_name}, current_time={current_time}")
    print(f"DEBUG: api_server.network is None: {api_server.network is None}")
    print(f"DEBUG: api_server.last_simulation_result is None: {api_server.last_simulation_result is None}")
    ...
    try:
        ...
        for route_idx, route in enumerate(routes):
            ...
            try:
                route_start = min(action.start_time for action in route.actions if hasattr(action, 'start_time'))
                route_end_times = [action.end_time for action in route.actions if hasattr(action, 'end_time') and action.end_time]
                if route_end_times:
                ...
                if current_dt < route_start or current_dt > route_end:
                    print(f"DEBUG: Route {route_idx} time range [{route_start}, {route_end}] doesn't include {current_dt}")
                    continue
            ...
            for action in route.actions:
                ...
                action_end = action.end_time if (hasattr(action, 'end_time') and action.end_time) else action_start
                ...
                if current_dt >= action_start and current_dt <= action_end:
                    ...
                    print(f"DEBUG: Route {route_idx} - found action {action.__class__.__name__} at time {current_dt} (range: {action_start} to {action_end})")
                    break
            ...
            if current_action:
                ...
                try:
                    unit_info = {
                        ...
                        "creation_time": route.actions[0].start_time.isoformat() if route.actions and hasattr(route.actions[0], 'start_time') else None,
                    }
                    
                    if isinstance(current_action, Wait):
                        if hasattr(current_action, 'location') and hasattr(current_action.location, 'hex_id'):
                            ...
                            if coords:
                                ...
                                print(f"DEBUG: Wait action - hex_id {hex_id} not found in hex_coords")
                    elif isinstance(current_action, Walk):
                        # Interpolate position during walk
                        if hasattr(current_action, 'start_time') and hasattr(current_action, 'end_time') and current_action.end_time:
                            total_duration = (current_action.end_time - current_action.start_time).total_seconds()
                            if total_duration > 0:
                                progress = (current_dt - current_action.start_time).total_seconds() / total_duration
                                progress = max(0, min(1, progress))  # Clamp between 0 and 1
                                
                                if hasattr(current_action, 'start_hex') and hasattr(current_action, 'end_hex'):
                                    ...
                                    if start_coords and end_coords:
                                        ...
                                        lng = start_coords[1] + (end_coords[1] - start_coords[1]) * progress
                                        unit_info["location"] = [lat, lng]
                                        unit_info["start_hex_id"] = start_hex_id
                                        unit_info["end_hex_id"] = end_hex_id
                                    else:
                                        print(f"DEBUG: Walk action - missing coords: start_hex={start_hex_id} (found: {start_coords is not None}), end_hex={end_hex_id} (found: {end_coords is not None})")
                                else:
                                    print(f"DEBUG: Walk action - missing start_hex or end_hex attributes")
                            else:
                        ...
                        if hasattr(current_action, 'vehicle') and current_action.vehicle:
                            ...
                            if hasattr(vehicle, 'timetable') and vehicle.timetable:
                                ...
                                for stop_idx, (arrival, departure) in vehicle.timetable.items():
                                    ...
                                    if stop_hex.hex_id in hex_coords:
                                        timetable_list.append({
                                            'stop_idx': stop_idx,
                                            'arrival': arrival.replace(tzinfo=None) if arrival.tzinfo else arrival,
                                            'departure': departure.replace(tzinfo=None) if departure.tzinfo else departure,
                                            'coords': hex_coords[stop_hex.hex_id]
                                        })
                                ...
                                for i, stop in enumerate(timetable_list):
                                    if current_dt >= stop['arrival'] and current_dt <= stop['departure']:
                                ...
                                if vehicle_position is None:
                                    for i in range(len(timetable_list) - 1):
                                        ...
                                        if current_dt >= stop1['departure'] and current_dt <= stop2['arrival']:
                                            # Calculate progress
                                            total_time = (stop2['arrival'] - stop1['departure']).total_seconds()
                                            elapsed = (current_dt - stop1['departure']).total_seconds()
                                            progress = max(0, min(1, elapsed / total_time)) if total_time > 0 else 0
                                            
                                            # Interpolate position
                                            lat = stop1['coords'][0] + (stop2['coords'][0] - stop1['coords'][0]) * progress
                                            lng = stop1['coords'][1] + (stop2['coords'][1] - stop1['coords'][1]) * progress
                    ...
                    if unit_info.get("location") is not None:
                        ...
                        print(f"DEBUG: Added unit {route_idx} at location {unit_info.get('location')} with action {unit_info.get('action_type')}")
                    else:
                        print(f"DEBUG: Skipping unit {route_idx} - no location (action: {unit_info.get('action_type')})")
                except Exception as e:
        ...
        print(f"DEBUG: Returning {len(units_data)} units with locations out of {len(routes)} total routes")
            ...
            for i, route in enumerate(routes[:3]):
                if hasattr(route, 'actions') and route.actions:
                    try:
                        ...
                        if route_end.tzinfo is not None:
                            route_end = route_end.replace(tzinfo=None)
                        print(f"  Route {i}: {route_start} to {route_end}, current_dt: {current_dt}")
                    except Exception as e:
            ...
            for action_idx, action in enumerate(route.actions):
                ...
                try:
                    event_queue_data.append({
                        ...
                        "end_time": action.end_time.isoformat() if (hasattr(action, 'end_time') and action.end_time) else None,
        ...
        print(error_details)
        raise HTTPException(status_code=500, detail=f"Failed to get simulation state: {str(e)}") from e
    ...
    try:
        ...
        for action_idx, action in enumerate(route.actions):
            step_info = {
                "index": action_idx,
                "type": action.__class__.__name__,
                "start_time": action.start_time.isoformat() if hasattr(action, 'start_time') else None,
                "end_time": action.end_time.isoformat() if (hasattr(action, 'end_time') and action.end_time) else None,
                "duration_minutes": action.duration_minutes if (hasattr(action, 'duration_minutes') and action.end_time) else None,
            }
        ...
        return {
            ...
            "total_time_minutes": route.time_taken_minutes if hasattr(route, 'time_taken_minutes') else None,
