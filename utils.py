...
def plot_network(G, geo_df, hour, selected_hex=None, demand_type='total', static=True):
    ...
    assert demand_type in ['total', 'depart', 'arrive'], 'demand_type must be "total" or "depart" or "arrive"'
    ...
    if hr not in geo_df.columns:
        raise ValueError(f"Column '{hr}' not found in GeoDataFrame. Available columns: {geo_df.columns.tolist()}")
    ...
    if not static:
        ...
        geo_df.plot(
            ...
            column=hr, cmap="Reds", legend=True, legend_kwds={"shrink": 0.4}
        )
