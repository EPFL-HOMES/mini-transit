...
class Network:
    ...
    def _get_available_bike(self, dock, service, demand_time):
        ...
        if hasattr(service, 'vehicles'):
            for v in service.vehicles:
                v_loc = self._safe_id(getattr(v, 'current_location', getattr(v, 'initial_location', None)))
    ...
    def get_distance(self, start_hex, end_hex) -> int:
        return self.get_cached_shortest_path_length(self._safe_id(start_hex), self._safe_id(end_hex))
    ...
    def compute_walk_time(self, graph, from_hex, to_hex, walk_speed):
        ...
        try:
            distance = nx.shortest_path_length(graph, source=source_id, target=target_id, weight="length")
    ...
    def find_closest_ondemand_vehicle(self, graph, hex_id, service, walk_speed, demand_time, radius=3):
        best_vehicle = None
        best_time = float("inf")
        ...
        try:
            nearby_hexes = nx.single_source_shortest_path_length(graph, source=source_id, cutoff=radius)
        except nx.NetworkXNoPath:
            return None, float("inf")
        ...
        for vehicle in service.vehicles:
            ...
            if target_id in nearby_hexes:
                ...
                walk_time_metric = (1 / (2 * walk_speed)) * math.sqrt(area / vehicle_within_radius_count)
    ...
    # =========================================================================
    def get_best_bike_to_bus_access(self, graph, start_hex, walk_speed, bike_speed, service, demand_time, radius=3):
        best_time = float('inf')
        ...
        for dock_s, dist_s in start_docks:
            ...
            for dock_d in service.docking_stations:
                ...
                if bike_dist == float('inf'): continue
    ...
    def get_best_bus_to_bike_egress(self, graph, end_hex, walk_speed, bike_speed, service, demand_time, radius=3):
        best_time = float('inf')
        best_res = (None, float('inf'), None, None)
        end_docks = []
        ...
        try:
            ...
            nearby_hexes = nx.single_source_shortest_path_length(graph, source=target_id, cutoff=radius)
        except nx.NetworkXNoPath:
            return best_res
        ...
        for dock_e, dist_e in end_docks:
            ...
            for dock_p in service.docking_stations:
                ...
                if dock_e_id == dock_p_id: continue
                if self._get_available_bike(dock_p, service, demand_time) is None: continue
    ...
    # =========================================================================
    # PUBLIC TRANSIT ROUTING LOGIC (UNCHANGED)
    # =========================================================================
    ...
    def build_closest_stops_table(self):
        ...
        try:
            distances, paths = nx.multi_source_dijkstra(self.graph, sources=all_stops, weight="length")
    ...
    def build_component_distance_table(self):
        ...
        self.component_distance_table = {
            "node_to_component": node_to_component, "table": table, "graph": component_graph,
        }
    ...
    def route_across_components_shortest_k(self, start_hex_id, end_hex_id, k=2):
        ...
        for comp_path in comp_paths:
            ...
            for c_from, c_to in zip(comp_path[:-1], comp_path[1:]):
                ...
                if s != a:
                    ...
                    if minutes > 0: used_fixed_route = True
            ...
            if s != e:
                ...
                minutes_end = 0
                if not used_fixed_route: continue
        ...
        self.path_lookup[self._safe_id(start_hex_id)][self._safe_id(end_hex_id)] = (all_chains, best_minutes)
        return all_chains
    ...
    def build_route_for_chain(self, chain, start, end, demand_time, demand, graph, walk_speed):
        ...
        return self.build_multimodal_chain_route(chain, start, end, demand_time, demand, graph, walk_speed, bike_speed, None, 'walk', 'walk', None, None)

    # =========================================================================
    # MULTIMODAL BUILDER
    # =========================================================================
    def build_multimodal_chain_route(self, chain, start, end, demand_time, demand, graph, walk_speed, bike_speed, docked_service, acc_mode, eg_mode, acc_data, eg_data):
        actions = []
        ...
        if acc_mode == 'bike':
            ...
            dock_s_obj, dock_d_obj = self._safe_hex(dock_s.location), self._safe_hex(dock_d.location)
            walk_time_s, walk_path_s = self.compute_walk_time(graph, start_obj, dock_s_obj, walk_speed)
            walk1 = Walk(start_time=current_time, end_time=current_time + timedelta(hours=walk_time_s),
                         start_hex=start_obj, end_hex=dock_s_obj, walk_speed=walk_speed, unit=demand.unit, graph=graph, walk_path=walk_path_s)
            actions.append(walk1)
            current_time = walk1.end_time

            vehicle = self._get_available_bike(dock_s, docked_service, current_time)
            if not vehicle: raise Exception("Bike became unavailable")
            
            bike_dist = self.get_cached_shortest_path_length(self._safe_id(dock_s_obj), self._safe_id(dock_d_obj))
            drive_time = bike_dist / bike_speed
            ride1 = OnDemandRide(start_time=current_time, end_time=current_time + timedelta(hours=drive_time),
                                 start_hex=dock_s_obj, end_hex=dock_d_obj, unit=demand.unit, service=docked_service, vehicle=vehicle)
            actions.append(ride1)
            current_time = ride1.end_time
            num_transfers += 1

            walk_time_b, walk_path_b = self.compute_walk_time(graph, dock_d_obj, self._safe_hex(s1_start), walk_speed)
            walk2 = Walk(start_time=current_time, end_time=current_time + timedelta(hours=walk_time_b),
                         start_hex=dock_d_obj, end_hex=self._safe_hex(s1_start), walk_speed=walk_speed, unit=demand.unit, graph=graph, walk_path=walk_path_b)
            actions.append(walk2)
            current_time = walk2.end_time
        else:
            s1_walk_time, walk_path = self.compute_walk_time(graph, start_obj, self._safe_hex(s1_start), walk_speed)
            if self._safe_id(start_obj) != s1_start:
                walk_to_start = Walk(start_time=current_time, end_time=current_time + timedelta(hours=s1_walk_time),
                                     start_hex=start_obj, end_hex=self._safe_hex(s1_start), walk_speed=walk_speed, unit=demand.unit, graph=graph, walk_path=walk_path)
        ...
        for i in range(len(chain) - 1):
            ...
            if curr_route == "WALK":
                walk_time, walk_path = self.compute_walk_time(graph, curr_hex_id, next_hex_id, walk_speed)
                t_walk = Walk(start_time=current_time, end_time=current_time + timedelta(hours=walk_time),
                              start_hex=self._safe_hex(curr_hex_id), end_hex=self._safe_hex(next_hex_id), walk_speed=walk_speed, unit=demand.unit, graph=graph, walk_path=walk_path)
                actions.append(t_walk)
                current_time = t_walk.end_time
                num_transfers += 1
            else:
                service = self.fixedroute_lookup.get(curr_route, None)
                wait_ride = service.get_route(unit=demand.unit, start_time=current_time, start_hex=self._safe_hex(curr_hex_id), end_hex=self._safe_hex(next_hex_id))
        ...
        if eg_mode == 'bike':
            ...
            dock_p_obj, dock_e_obj = self._safe_hex(dock_p.location), self._safe_hex(dock_e.location)
            walk_time_p, walk_path_p = self.compute_walk_time(graph, prev_hex_id, dock_p_obj, walk_speed)
            walk3 = Walk(start_time=current_time, end_time=current_time + timedelta(hours=walk_time_p),
                         start_hex=self._safe_hex(prev_hex_id), end_hex=dock_p_obj, walk_speed=walk_speed, unit=demand.unit, graph=graph, walk_path=walk_path_p)
            actions.append(walk3)
            current_time = walk3.end_time

            vehicle = self._get_available_bike(dock_p, docked_service, current_time)
            if not vehicle: raise Exception("Bike became unavailable at egress")
            bike_dist = self.get_cached_shortest_path_length(self._safe_id(dock_p_obj), self._safe_id(dock_e_obj))
            drive_time = bike_dist / bike_speed
            ride2 = OnDemandRide(start_time=current_time, end_time=current_time + timedelta(hours=drive_time),
                                 start_hex=dock_p_obj, end_hex=dock_e_obj, unit=demand.unit, service=docked_service, vehicle=vehicle)
            actions.append(ride2)
            current_time = ride2.end_time
            num_transfers += 1

            walk_time_e, walk_path_e = self.compute_walk_time(graph, dock_e_obj, end_obj, walk_speed)
            walk4 = Walk(start_time=current_time, end_time=current_time + timedelta(hours=walk_time_e),
                         start_hex=dock_e_obj, end_hex=end_obj, walk_speed=walk_speed, unit=demand.unit, graph=graph, walk_path=walk_path_e)
            actions.append(walk4)
        else:
            final_walk_time, walk_path = self.compute_walk_time(graph, prev_hex_id, end_obj, walk_speed)
            if prev_hex_id != self._safe_id(end_obj):
                final_walk = Walk(start_time=current_time, end_time=current_time + timedelta(hours=final_walk_time),
                                  start_hex=self._safe_hex(prev_hex_id), end_hex=end_obj, walk_speed=walk_speed, unit=demand.unit, graph=graph, walk_path=walk_path)
    ...
    def get_optimal_route(self, demand, second_try=False):
        ...
        if walk_time < float("inf"):
            walk_action = Walk(start_time=demand_time, start_hex=start, end_hex=end, unit=demand.unit, 
                               graph=self.graph, walk_speed=walk_speed, walk_path=walk_path, 
        ...
        try:
            ...
            chains = self.route_across_components_shortest_k(start_stop, end_stop, k=2) if start_stop and end_stop else []
            for chain in chains:
                if chain is None or len(chain) == 0: continue
                try:
                    fixedroute_chain_route = self.build_route_for_chain(chain, start, end, demand_time, demand, self.graph, walk_speed)
        ...
        for service in ondemandservices_docked:
            ...
            try:
                nearby_start_hexes = nx.single_source_shortest_path_length(self.graph, source=self._safe_id(start), cutoff=3)
                for dock in service.docking_stations:
                    dock_id = self._safe_id(dock.location)
            ...
            if not start_docks_with_bikes: continue
            ...
            try:
                ...
                nearby_end_hexes = nx.single_source_shortest_path_length(self.graph, source=self._safe_id(end), cutoff=3)
                for dock in service.docking_stations:
                    dock_id = self._safe_id(dock.location)
            ...
            for s_dock, s_dist in start_docks_with_bikes:
                for e_dock, e_dist in end_docks_with_space:
                    try:
                        ...
                        if self._safe_id(b_start_loc) == self._safe_id(b_end_loc): continue
                        ...
                        if self._safe_id(start) != self._safe_id(b_start_loc):
                            ...
                            walk_to_vehicle = Walk(start_time=demand_time, end_time=demand_time + timedelta(hours=vehicle_walk_time),
                                                   start_hex=start, end_hex=b_start_loc, unit=demand.unit,
                                                   graph=self.graph, walk_speed=walk_speed, walk_path=walk_path)
                            actions.append(walk_to_vehicle)
                            
                        # PERFORMANCE FIX: Cached distance
                        bike_dist = self.get_cached_shortest_path_length(self._safe_id(b_start_loc), self._safe_id(b_end_loc))
                        if bike_dist == float('inf'): continue
                        drive_time = bike_dist / bike_speed
                        arrival_time = actions[-1].end_time + timedelta(hours=drive_time) if actions else demand_time + timedelta(hours=drive_time)
                        
                        ride_action = OnDemandRide(start_time=actions[-1].end_time if actions else demand_time, end_time=arrival_time,
                                                   start_hex=b_start_loc, end_hex=b_end_loc,
                                                   unit=demand.unit, service=service, vehicle=vehicle)
                        ...
                        if self._safe_id(b_end_loc) != self._safe_id(end):
                            ...
                            walk_from_vehicle = Walk(start_time=ride_action.end_time, end_time=ride_action.end_time + timedelta(hours=off_vehicle_walk_time),
                                                     start_hex=b_end_loc, end_hex=end, unit=demand.unit,
                                                     graph=self.graph, walk_speed=walk_speed, walk_path=walk_path)
                            actions.append(walk_from_vehicle)
                            
                        ondemand_route = Route(unit=demand.unit, actions=actions, transfers=0, config=self.config)
        ...
        for service in ondemandservices_dockless:
            best_vehicle, vehicle_walk_time = self.find_closest_ondemand_vehicle(self.graph, start, service, walk_speed, demand_time, radius=3)
            ...
            try:
                ...
                if self._safe_id(start) != self._safe_id(v_loc):
                    ...
                    walk_to_vehicle = Walk(start_time=demand_time, end_time=demand_time + timedelta(hours=vehicle_walk_time),
                                           start_hex=start, end_hex=v_loc, unit=demand.unit,
                                           graph=self.graph, walk_path=walk_path, walk_speed=walk_speed)
                    actions.append(walk_to_vehicle)
                    
                # PERFORMANCE FIX: Cached distance
                bike_dist = self.get_cached_shortest_path_length(self._safe_id(v_loc), self._safe_id(end))
                if bike_dist == float('inf'): continue
                ...
                ride_action = OnDemandRide(start_time=actions[-1].end_time if actions else demand_time, end_time=arrival_time,
                                           start_hex=v_loc, end_hex=end, unit=demand.unit,
                                           service=service, vehicle=best_vehicle)
                actions.append(ride_action)
        ...
        if fixed_services and ondemandservices_docked:
            ...
            b_acc = self.get_best_bike_to_bus_access(self.graph, start, walk_speed, bike_speed, docked_srv, demand_time, radius=3)
            b_egr = self.get_best_bus_to_bike_egress(self.graph, end, walk_speed, bike_speed, docked_srv, demand_time, radius=3)
            ...
            for acc_mode, eg_mode, B_S, B_E, acc_data, eg_data in combos:
                chains = self.route_across_components_shortest_k(B_S, B_E, k=1) if B_S and B_E else []
                for chain in chains:
                    if chain is None or len(chain) == 0: continue
                    try:
                        rt = self.build_multimodal_chain_route(chain, start, end, demand_time, demand, self.graph, walk_speed, bike_speed, docked_srv, acc_mode, eg_mode, acc_data, eg_data)
        ...
        for route in raw_options:
            if route is not None:
                ...
                if signature not in unique_choices or route.total_cost > unique_choices[signature].total_cost:
