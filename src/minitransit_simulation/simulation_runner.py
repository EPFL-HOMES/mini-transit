    ...
    def run_simulation(self, runner_input: SimulationRunnerInput) -> SimulationRunnerResult:
        ...
        try:
            ...
            if start_hour > end_hour:
                return SimulationRunnerResult(
                    status="error", message="start_hour cannot be greater than end_hour", routes=[]
                )

            use_sampling = getattr(self.config, "sampling", True)
            print(
                f"DEBUG: Running simulation from {start_hour}:00 to {end_hour}:00. Sampling: {use_sampling}"
            )
            ...
            for route in routes:
                ...
                for action in route.actions:
                    ...
                    if act_type == "OnDemandRide" or action_data.get("type") == "OnDemandRide":
                        ...
                        if not action_path and start_id != "N/A" and end_id != "N/A":
                            try:
                                ...
                                action_path = nx.shortest_path(
                                    self.network.graph, source=start_id, target=end_id
                                )
                            except Exception:
                        ...
                        action_data["distance"] = (
                            len(action_data["path"]) - 1 if action_data["path"] else 0
                        )
                        # =========================================================
                        ...
                        if "start_hex" not in action_data:
                            action_data["start_hex"] = loc_id
                        if "end_hex" not in action_data:
                            action_data["end_hex"] = loc_id
                    # ---------------------------------------------------------
