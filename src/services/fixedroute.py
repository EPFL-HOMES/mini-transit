...
class FixedRouteService(Service):
    ...
    def __init__(
        ...
        self.vehicles = [
            FixedRouteVehicle(self, timetable=t) for t in vehicles
    ...
    def get_route(self, unit, start_time: datetime, start_hex: Hex, end_hex: Hex) -> Route:
        ...
        ride_action = Ride(next_departure, arrival_time, start_hex, end_hex, unit, service=self)
...
class FixedRouteVehicle:
    ...
    def __init__(
        ...
        self.timetable = (
            timetable  # OrderedDict mapping stop index to (arrival_time, departure_time)
        )
    ...
    def __verify_timetable(self):
        ...
        for i in range(len(self.timetable) - 1):
            ...
            if next_arrival_time - departure_time < expected_travel_time:
                raise ValueError(
                    "Inconsistent timetable between stops {} and {}".format(start_stop, end_stop)
                )
