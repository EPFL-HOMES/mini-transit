...
class APIServer:
    ...
    def init_app(self, city_name: str):
        ...
        print(
            f"Initialized {city_name}: {len(self.demands)} demands, {len(self.network.graph.nodes())} hexagons"
        )
                ...
                for action in route.actions:
                    ...
                    if hasattr(action, "start_time"):
                        ...
                        action_data = {
                            ...
                            "start_time": action["start_time"].isoformat(),
                            "end_time": (
                                action["end_time"].isoformat() if action.get("end_time") else None
                            ),
                            "duration_minutes": (
    ...
    def _save_simulation_results(self, result: dict, simulation_hour: int):
        ...
        try:
            ...
            print(
                f"Starting to save simulation results for {self.city_name} hour {simulation_hour}"
            )
